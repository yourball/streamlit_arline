...
def run_experiment(target, pipelines_list):
...
def get_random_targ_cfg(num_qubits, num_gates, circ_type, circ_options, density_cx):
...
def get_results():
    ...
    compilers_list = []
    if add_qiskit:
        compilers_list.append(QISKIT_NAME)
    if add_tket:
        compilers_list.append(TKET_NAME)
    if add_pyzx:
        compilers_list.append(PYZX_NAME)
    if add_voqc:
        compilers_list.append(VOQC_NAME)
    num_compilers = len(compilers_list)
    ...
    circ_type = radio_group_circ_type()

    q_dynamics_target = 'Trotter' in circ_type

    if 'Random' in circ_type:
    ...
    if routing_only:
        ...
        voqc_stages = [target_analysis, unroll, voqc_mapping]
    else:
    ...
    proceed = True
    target = None
    optimized_qasm_list = None

    if ('Random' in circ_type) and num_qubits_hardw < num_qubits_circ:
    ...
    if click:
        ...
        if 'Random' in circ_type:
            targ_cfg = get_random_targ_cfg(num_qubits_circ, num_gates, circ_type, circ_options, density_cx)
        ...
        if proceed:
            df_1q, df_2q, df_tcount, df_time, df_check, df_full, optimized_qasm_list = run_experiment(target, pipelines_list)
    ...
    return target, optimized_qasm_list, compilers_list

if __name__ == "__main__":
    im = Image.open("arline.png")
    st.set_page_config(page_title='ArlineQ', page_icon=im, layout="wide",)

    target, optimized_qasm_list, compilers_list = get_results()
    cols = st.columns(len(compilers_list)+1)
    ...
    if optimized_qasm_list is not None:
        for i in range(len(compilers_list)):
            compiler_name = compilers_list[i]
            with cols[i+1]:
                st.download_button(
